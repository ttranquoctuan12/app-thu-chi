...
# ==================== CÁC VIEW RENDER (ĐÃ SẮP XẾP LẠI THỨ TỰ) ====================

def render_dashboard_box(bal, thu, chi):
    text_color = "#2ecc71" if bal >= 0 else "#e74c3c"
    # CHỈNH SỬA: ĐƯA CHỮ KÝ RA NGOÀI, GÓC TRÁI DƯỚI
    html_content = f"""
...
def render_input_form():
    with st.container(border=True):
        ...
        def auto_fill_callback():
            if "công tác phí" in st.session_state.desc_new.lower():
                st.session_state.a_new = 150000; st.session_state.t_new = "Chi"; st.toast("💡 Đã tự động điền 150k công tác phí!")
        ...
        if not row_to_edit.empty:
            ...
            with st.container(border=True):
                ...
                if b1.button("💾 LƯU", type="primary", use_container_width=True):
                    update_transaction(st.session_state.edit_row_index, ud_date, ud_type, ud_amt, ud_desc, row_data['HinhAnh']); st.session_state.edit_row_index = None; st.rerun()
...
def render_vattu_module():
    ...
    with st.container(border=True):
        ...
        if proj_name_input:
            ...
            vt_unit1 = c1.text_input("ĐVT Cấp 1 (Lớn):", value=u1, placeholder="VD: Cuộn")
            vt_unit2 = c2.text_input("ĐVT Cấp 2 (Nhỏ):", value=u2, placeholder="VD: Mét")
            ...
            if vt_unit1 and vt_unit1 in sel_unit_label:
                sel_unit = vt_unit1; final_price_suggest = vt_price1
...
st.markdown("<div class='app-footer'>Phiên bản: 4.2 Stable Fix - Powered by TUẤN VDS.HCM</div>", unsafe_allow_html=True)
